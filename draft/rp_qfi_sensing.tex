...
repository.\footnote{\url{https://github.com/arturoornelasb/rp-qfi-sensing}; experiments
\texttt{q01}--\texttt{q05}, each with its frozen reading in the script header.}
...
sharpening of the gasket null (registered as Q3c); (iii) an atomic GOE$\to$GUE spacing
measurement --- no atomic system has ever shown that crossover in level statistics, and the
mechanical version is now fully characterized in-silico~\cite{ornelas2026fem}. A dedicated
feasibility review (2026-07-11, this program's research record) finds the atomic kicked-rotor
route \emph{not yet} statistically feasible: the bottleneck is resolved-level count (tens of
levels demonstrated on the closest platforms, against the $\sim$50--100$+$ a spacing histogram
needs), while the two other ingredients exist separately --- quantum-resonance delocalization is
a proven mechanism, and a genuine time-reversal-control knob has been demonstrated on a cold-atom
Floquet platform, though verified only through transport observables, never through quasienergy
spectroscopy. No platform yet combines all three; the prediction is thereby a defined engineering
gap rather than an open guess, and as of this writing the field is clean --- the closest adjacent
work computes genuine spacing statistics on a kicked rotor but explicitly remains in the
orthogonal class. (iv) The composition claimed in this note --- multifractal dimension as a
metrological figure of merit --- likewise remains unclaimed in the 2024--2026 literature at the
time of deposit, though its ingredients are public and recent~\cite{sak2022,hu2026}; priority is
the point of depositing now.
